...
$rowsPerPage = 50;
$pageNum = 1;
$self ="manage_my_patient.php";
$root = "manage_my_patient.php";
$flag = false;
if(isset($_GET['key']))
{
    $key = trim($_GET['key']);
                                ...
                                <div class="row">
                                    <div class="col-12 text-sm-center form-inline" >
                                        ...
                                        <div class="form-group">
                                            <form method="get" action="manage_my_inpatient.php">
                            ...
                            <div class="table-responsive">
                                <table id="" class="table table-bordered toggle-circle mb-0" data-page-size="7">
                                    ...
                                    */
                                    if($flag)
                                    {
                                        $sql= "SELECT * FROM `treatment` T JOIN `cure` C ON C.TreatCode = T.TreatCode JOIN `inpatient` I ON T.IPCode = I.IPCode JOIN `patient` P ON P.PCode = I.PCode  WHERE (I.IPCode = '".$key."' OR P.Fname LIKE '%".$key."%' OR P.Lname LIKE '%".$key."%') AND C.DECode = '".$doc_id."' OR CONCAT(Fname,' ',Lname) LIKE '%".$key."%' GROUP BY T.IPCode";
                                    }else
                                        $sql= "SELECT * FROM `treatment` T JOIN `cure` C ON C.TreatCode = T.TreatCode JOIN `inpatient` I ON T.IPCode = I.IPCode JOIN `patient` P ON P.PCode = I.PCode  WHERE C.DECode = '".$doc_id."' GROUP BY T.IPCode";
