<?php
    ...
    date_default_timezone_set('Asia/Ho_Chi_Minh');
		...
		if(isset($_POST['add_patient_presc']))
		{
            ...
            $sql = "SELECT * FROM `outpatient` WHERE `PCode` = '".$pres_pat_number."'";
            $result = executeQuery($sql);
            if($result->num_rows == 0)
            {
                $sql = "SELECT * FROM `outpatient` ORDER BY PCode DESC";
                $result = executeQuery($sql);
                if($result->num_rows == 0)
                {
                    $id = "OP000000001";
                }else
                {
                    ...
                    $max =  (int)substr($row['OPCode'],2);
                    while($row = $result -> fetch_array())
                    {
                        if($max < (int)substr($row['OPCode'],2) )
                            $max = (int)substr($row['OPCode'],2);
                    }
                ...
                $sql = "insert into outpatient (OPCode,PCode) values ('$id', '".$pres_pat_number."')";
                $result = executeQuery($sql);
                $OPCode = $id;
            }else
            {
            ...
            $sql5 = "INSERT INTO `examination` (`ExamCode`, `Diagnosis`, `ExamDate`, `NextExDate`, `Fee`, `DECode`, `OPCode`) VALUES ('".$pres_number."','".$pres_pat_ailment."','".date("Y-m-d H:i:s")."','".$next_date."','".$fee."','".$_SESSION['doc_id']."','".$OPCode."')";
            ...
            if($flag)
            {
                for($i = 0; $i < count($med); $i++)
                {
                    ...
                    $row3 = $result3-> fetch_array();
                    $price = $row3['Price'];
                    $sql4 = "INSERT INTO `exam_use` (`ExamCode`, `MCode`, `quantity`, `price`) VALUES ('".$pres_number."', '".$med[$i]."', '".$quan[$i]."', '".$price."')";
                    executeQuery($sql4);
                }
            }
                                            ...
                                            <form method="post">
                                                ...
                                                <div class="form-row">
                                                    ...
                                                    <div class="form-group col-md-6 ">
                                                        ...
                                                        <input required="required" type="text" value="" name="pres_pat_ailment" class="form-control" id="inputCity">
                                                    </div>
                                                    <div class="form-group col-md-6">
                                                        <label for="inputPassword4" class="col-form-label">Re-examination date
                                                        </label>
                                                        <input type="date"  name="re_exam" value="" class="form-control"  id="inputPassword4" placeholder="Patient`s Last Name">
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-row">
                                                    ...
                                                    <div class="form-group col-md-2" style="display:none">
                                                        ...
                                                        $sql2 = "SELECT * FROM `examination` ORDER BY ExamCode DESC";
                                                        $result2 = executeQuery($sql2);
                                                        $pres_no = "EX000000001";
                                                        if($result2->num_rows > 0)
                                                        {
                                                            $row2 = $result2 -> fetch_array();
                                                            $max = (int)substr($row2['ExamCode'],2);
                                                            while($row2 = $result2 -> fetch_array())
                                                            {
                                                                if($max <= (int)substr($row2['ExamCode'],2))
                                                                    $max = (int)substr($row2['ExamCode'],2);
                                                            }
